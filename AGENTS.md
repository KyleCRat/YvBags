...
- Item ID, Item Name, and Tooltip Text Rules may contain multiple alternatives within one atomic Rule. Equals and Contains match any complete alternative; Does Not Equal and Does Not Contain negate that grouped result and therefore match only when none of the alternatives match. Empty alternatives are ignored. Item IDs are positive integers compiled into a lookup set; preserve legacy scalar IDs and keep ID rules equality-only.
- Ordered fields support strict Greater Than/Less Than and inclusive At Least/At Most comparisons. Profession Quality rules compare numeric ranks across quality systems; paired rank icons are presentation only, not distinct stored values.
